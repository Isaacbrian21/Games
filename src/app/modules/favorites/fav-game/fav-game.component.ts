...
export class FavGameComponent implements OnInit {
  ...
  constructor(private favoriteService: FavoriteService, private confirmationService: ConfirmationService, private primengConfig: PrimeNGConfig, private messageService: MessageService ) {
    this.getFavs()
   }
  ...
  ngOnInit(): void {
    ...
    confirm2(favorito: Favorito) {
        this.confirmationService.confirm({
            ...
            reject: () => {
                this.messageService.add({severity:'error', summary:'Nada feito!', detail:`O ${favorito.name} não foi emovido dos seus favoritos`});
            }
        });
    }
  ...
  getFavs(): void{
    this.favoriteService.getFav().subscribe((fav) =>{this.favoritos = fav; console.log(fav);
